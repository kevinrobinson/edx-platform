...
class TestProgramRetrieval(ProgramsApiConfigMixin, ProgramsDataMixin,
    ...
    def test_get_engaged_programs(self):
        """Verify that programs are returned in the correct order when the user has multiple enrollments."""
        self.create_programs_config()
        self.mock_programs_api()
        ...
        # The most recently created enrollment corresponds to the last test program.
        expected = programs[::-1]

        self.assertEqual(expected, actual)

    @httpretty.activate
    def test_get_engaged_programs_enrolled_subset(self):
        """Verify that programs are returned when the user has a single enrollment."""
        self.create_programs_config()
        self.mock_programs_api()
