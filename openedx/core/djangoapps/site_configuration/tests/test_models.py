"""
Tests for comprehensive theme related django models.
"""
...
class SiteConfigurationTests(TestCase):
    """
    Test SiteConfiguration model and its receivers.
    """
    ...
    def test_site_configuration_post_save_receiver(self):
        """
        Test that and entry is added to SiteConfigurationHistory Model each time a record in
        SiteConfiguration models is added.
        """
    ...
    def test_site_configuration_post_update_receiver(self):
        """
        Test that and entry is added to SiteConfigurationHistory Model each time a record in
        SiteConfiguration models is updated.
        """
        # add SiteConfiguration to database
        site_configuration = SiteConfigurationFactory.create(
            site=self.site,
    ...
    def test_no_entry_is_saved_for_errors(self):
        """
        Test that and entry is not added to SiteConfigurationHistory Model if there is an error while
        saving SiteConfiguration updated.
        """
        # add SiteConfiguration to database
        site_configuration = SiteConfigurationFactory.create(
            site=self.site,
