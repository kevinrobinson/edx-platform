...
class AppVersionUpgrade(object):
    ...
    def process_request(self, request):
        ...
        if version_data:
            last_supported_date = version_data.get(self.UPGRADE_DEADLINE_HEADER, None)
    ...
    def process_response(self, _request, response):
        ...
        if version_data:
            upgrade_deadline = version_data.get(self.UPGRADE_DEADLINE_HEADER, None)
            ...
            latest_version = version_data.get(self.LATEST_VERSION_HEADER, None)
            user_app_version = version_data.get(self.USER_APP_VERSION, None)
            if (latest_version and latest_version != self.NO_LATEST_VERSION and
    ...
    def get_info_for_version(self, request):
        ...
        if platform:
            ...
            last_supported_date = cached_data.get(self.get_cache_key_name(
                user_agent,
                self.UPGRADE_DEADLINE_HEADER),
                None)
            ...
            latest_version = cached_data.get(self.get_cache_key_name(platform.NAME, self.LATEST_VERSION_HEADER), None)
